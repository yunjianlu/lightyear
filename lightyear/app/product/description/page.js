...
function StarRating({ rating }) {
  const stars = Array.from({ length: Math.floor(rating) }, () => "⭐").join("");
  return <span>{stars}</span>;
}

// Isolate all client-side hooks into this component
export default function DetailedProductPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const productIdParam = searchParams.get("id");

  if (!productIdParam) return <p>Invalid product ID</p>;

  const product = products.find((p) => p.productId === productIdParam);
      ...
      <div className="p-8 grid grid-cols-1 md:grid-cols-3 md:gap-2 grid-flow-row md:grid-flow-col md:grid-rows-[min-content_min-content] h-min">
        ...
        <div id="leftScrollStick" className="row-span-2">
          <div className="grid grid-cols-1 sm:grid-cols-3 grid-rows-[8rem_minmax(15rem,32rem)_auto_auto_auto] sm:grid-rows-[8rem_minmax(15rem,32rem)_auto_auto] md:grid-rows-[8rem_minmax(15rem,26rem)_auto_auto] bg-white mb-2">
            ...
            <div className="col-span-1 sm:row-span-1 flex justify-center items-center whitespace-pre-wrap">
              <p>
                {product.starRating
                  ? `${product.starRating} <StarRating rating={product.starRating}/> (${product.numberOfReviews}) reviews`
                  : "Unknown Rating"}
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 grid-flow-row bg-black gap-y-0.25 auto-rows-min h-min mb-2 md:mb-0">
            ...
            <div className="flex justify-center bg-white">
              <p className="m-auto">{product.orderedQuantityPastMonth || 0}</p>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <div className="grid grid-flow-row bg-black grid-cols-2 gap-y-0.25 auto-rows-min h-min mb-2 md:mb-0 md:col-span-1">
            ...
            <SpecificationRow title="Product ID" value={product.productId} />
            <SpecificationRow title="Vendor" value={product.vendor} />
            <SpecificationRow
              title="Dimensions"
            ...
            <SpecificationRow
              title="Weight"
              value={product.productDetails.weight}
            />
            <SpecificationRow
              title="Place of Origin"
              value={product.productDetails.originLocation}
            />
            <SpecificationRow
              title="Batteries Included"
              value={product.productDetails.batteriesIncluded}
            />
            <SpecificationRow
              title="Grogu Approved?"
              value={product.productDetails.groguApproved ? "Yes" : "No"}
            />
          </div>
...
// Small helper for specification rows
function SpecificationRow({ title, value }) {
  return (
    <>
      <h2 className="bg-white pl-2">{title}</h2>
      <div className="flex justify-center bg-white">
        <p>{value || "Unknown"}</p>
      </div>
    </>
